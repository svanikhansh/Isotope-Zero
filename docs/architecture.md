...
`HAVE_NATIVE is False` + version == tag) and a `smoke-npm` job
(npm-installs the just-published launcher and runs `izero --version`). The
build-time test asserts the wheel imports in the *build* environment; the
smoke job closes the gap between "wheel builds" and "users
