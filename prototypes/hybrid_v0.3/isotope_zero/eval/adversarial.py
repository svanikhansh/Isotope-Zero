...
class ScaleResult:
    ...
    rss_mb: float = 0.0
    # Hybrid BM25+FTS5 pre-filter + vector re-ranking (Method 1). vector_p99_ms
    # above IS the full-matrix scan and is NOT repurposed; hybrid_* are the
    # two-pass pipeline latencies measured on the same store/cards/idx.
    hybrid_p50_ms: float = 0.0
    hybrid_p95_ms: float = 0.0
    hybrid_p99_ms: float = 0.0
    claim_vector_ms: float = 2.0
    claim_sql_ms: float = 0.8
    claim_rss_mb: float = 200.0
    claim_hybrid_ms: float = 0.05  # Method 1 target: sub-0.05ms hybrid p99
    ...
    @property
    def hybrid_claim_holds(self) -> bool:
        return self.hybrid_p99_ms < self.claim_hybrid_ms
...
class AdversarialResult:
    ...
    # Needle recall via hybrid_vector_search directly (isolated from the router's
    # lexical boost / truncation). None until run_needle_hybrid is wired in.
    needle_hybrid: NeedleResult | None = None
...
def run_scale(n: int, embedder: EmbeddingEngine, reps: int = 30) -> ScaleResult:
    ...
    # Hybrid (BM25+FTS5 pre-filter + vector re-ranking) warm-up on the SAME
    # store/cards/idx. Using the card's own fact as query_text is BEST-CASE for
    # the lexical filter (exact token overlap) — fine for the latency ceiling
    # microbenchmark; the needle run below is the honest recall probe.
    q_text = cards[idx].fact
    store.hybrid_vector_search(q_text, q_vec, top_k=5, candidate_limit=50, alpha=0.3)
    v, s2, h = [], [], []
    for _ in range(reps):
        ...
        t0 = time.perf_counter(); store.hybrid_vector_search(q_text, q_vec, top_k=5, candidate_limit=50, alpha=0.3); h.append(time.perf_counter() - t0)
    vp = _percentiles(v, (50, 95, 99)); sp = _percentiles(s2, (50, 95, 99))
    hp = _percentiles(h, (50, 95, 99))
    ...
    return ScaleResult(n_cards=n, vector_p50_ms=vp[0], vector_p95_ms=vp[1], vector_p99_ms=vp[2],
                       ...
                       hybrid_p50_ms=hp[0], hybrid_p95_ms=hp[1], hybrid_p99_ms=hp[2])
...
def run_needle_hybrid(n_distractors: int, embedder: EmbeddingEngine, reps: int = 100) -> NeedleResult:
    """Needle recall via hybrid_vector_search DIRECTLY (no router).

    Mirrors run_needle exactly — same NEEDLE_FACT, same NEEDLE_QUERIES, same
    distractors via _make_distractors, same "2204"-in-results hit criterion —
    but calls store.hybrid_vector_search(q, qv, ...) instead of router.query.
    This isolates the hybrid search's recall from the router's lexical boost /
    truncation, so a recall drop is attributable to BM25 pre-filtering alone.
    The semantic NEEDLE_QUERIES ("What port is the SSH key on?") do NOT contain
    the literal "2204", so this is the honest probe of whether the lexical
    pre-filter can survive semantic-only queries.
    """
    store = MemoryStore(":memory:", embedder=embedder)
    needle = MemoryCard(id="needle-ssh", fact=NEEDLE_FACT,
                        evidence="ops said: 'ssh key port 2204 not 22'", timestamp=now_ts(),
    ...
    hits = 0
    for q in NEEDLE_QUERIES[:reps]:
        qv = _norm(embedder.embed_text(q))
        res = store.hybrid_vector_search(q, qv, top_k=5, candidate_limit=50, alpha=0.3)
        blob = " ".join(card.fact for (card, _score) in res).lower()
        if "2204" in blob:
            hits += 1
    recall = (hits / min(reps, len(NEEDLE_QUERIES))) * 100.0 if reps else 0.0
    store.close()
...
def run_adversarial(n_scale: int = 10_000, n_distractors: int = 500,
    ...
    needle_hybrid = run_needle_hybrid(n_distractors, backend, reps=n_needle_queries)
    ...
    return AdversarialResult(
        scale=scale, needle=needle, needle_hybrid=needle_hybrid,
        negation=negation, concurrency=concurrency,
...
def render_adversarial_markdown(res: AdversarialResult) -> str:
    ...
    lines.append("|---|---|---|---|")
    lines.append(f"| vector_search p99 (full-matrix) | {s.vector_p99_ms:.2f} ms | <{s.claim_vector_ms} ms | {'PASS' if s.vector_claim_holds else 'FAIL'} |")
    lines.append(f"| hybrid_vector_search p99 (BM25+FTS5 pre-filter) | {s.hybrid_p99_ms:.3f} ms | <{s.claim_hybrid_ms} ms | {'PASS' if s.hybrid_claim_holds else 'FAIL'} |")
    ...
    lines.append(f"vector p50/p95/p99 = {s.vector_p50_ms:.2f}/{s.vector_p95_ms:.2f}/{s.vector_p99_ms:.2f} ms; "
                 f"hybrid p50/p95/p99 = {s.hybrid_p50_ms:.3f}/{s.hybrid_p95_ms:.3f}/{s.hybrid_p99_ms:.3f} ms; "
    ...
    lines.append(f"- recall via router (QueryRouter.query): **{n.recall_pct:.1f}%** (claim {n.claim_recall_pct}%) → "
                 f"{'PASS' if n.recall_claim_holds else 'FAIL'}")
    if res.needle_hybrid is not None:
        nh = res.needle_hybrid
        lines.append(f"- recall via hybrid (hybrid_vector_search direct): **{nh.recall_pct:.1f}%** (claim {nh.claim_recall_pct}%) → "
                     f"{'PASS' if nh.recall_claim_holds else 'FAIL'}")
        lines.append(f"- recall delta router→hybrid: {n.recall_pct - nh.recall_pct:+.1f} pts")
    lines.append("")
    ...
    lines.append("")
    hybrid_recall_hold = res.needle_hybrid.recall_claim_holds if res.needle_hybrid is not None else False
    passed = sum([s.vector_claim_holds, s.hybrid_claim_holds, s.sql_claim_holds, s.rss_claim_holds,
                  n.recall_claim_holds, hybrid_recall_hold, g.negation_claim_holds, c.concurrency_claim_holds])
    total_claims = 8 if res.needle_hybrid is not None else 7
    lines.append(f"**Total: {passed}/{total_claims} claims hold.** (harness wall-clock {res.latency_ms/1000.0:.1f}s)")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        import json
        needle_hybrid_obj = None
        if res.needle_hybrid is not None:
            needle_hybrid_obj = {**res.needle_hybrid.__dict__,
                                 "recall_claim_holds": res.needle_hybrid.recall_claim_holds}
        print(json.dumps({
            "scale": {**res.scale.__dict__, "vector_claim_holds": res.scale.vector_claim_holds,
                      "hybrid_claim_holds": res.scale.hybrid_claim_holds,
            ...
            "needle_hybrid": needle_hybrid_obj,
