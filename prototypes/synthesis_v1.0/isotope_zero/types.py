...
class MemoryCard:
    ...
    stability: float = 1.0      # Ebbinghaus stability S (higher = slower decay)
    importance: float = 0.0     # User-set importance flag [0.0, 1.0]
    archived: float = 0.0       # 0.0 = live; > 0.0 = archived at this timestamp
    # Multi-tier scoping: isolate memories by user_id/agent_id/run_id boundaries.
    # Deterministic scope string format: "user_id=123&agent_id=456&run_id=789"
    scope: str = "default"
    # mem0 port (dedup + TTL): content-aware fingerprint for exact-duplicate
    # detection (ports mem0's hashlib.md5 hash at mem0/memory/main.py:1010 to
    # a content-aware sha256 — see isotope_zero.core.dedup). ``None`` means
    # "not yet computed"; the store sets it on ``add()``.
    content_fingerprint: str | None = None
    # Optional time-to-live: when set, the store computes
    # ``expiration_timestamp = timestamp + ttl_seconds`` and excludes the card
    # from retrieval once ``now > expiration_timestamp``. ``None`` (default)
    # means "never expires" — the pre-TTL behaviour, preserving backward
    # compatibility for every caller that doesn't pass it.
