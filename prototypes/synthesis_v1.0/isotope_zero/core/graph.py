...
# Optional NumPy accelerator. The graph layer is stdlib-first by design
# (see module docstring); when numpy is importable the bulk pairwise-cosine
# path uses a single vectorized ``M @ M.T`` matmul instead of the per-pair
# pure-Python loop. Falls back transparently so the module stays importable
# with zero dependencies — mirroring the store's own numpy fast-path.
...
def _pairwise_cosine(ids: list[str], embs: list[list[float]]) -> dict[tuple[str, str], float]:
    """Pairwise cosine similarity for a batch of (assumed unit-length) embeddings.

    Returns a ``{(id_i, id_j): score}`` dict for ``i < j`` pairs only (upper
    triangle — the graph is undirected and ``auto_link_cards`` writes a directed
    edge per pair anyway). This is the vectorized heart of the bulk-seed path:
    a single ``M @ M.T`` matmul over the ``(n, dim)`` matrix replaces the naive
    O(n²) pure-Python double loop that ``auto_link_cards`` ran per card.

    When numpy is unavailable, falls back to the pure-Python pairwise loop so
    the bulk path still works (just slower) with zero dependencies.
    """
    n = len(ids)
    if n < 2:
        return {}
    out: dict[tuple[str, str], float] = {}
    if _HAS_NUMPY and embs:
        dim = len(embs[0])
        M = _np.asarray(embs, dtype=_np.float32).reshape(n, dim)
        # Cosine = dot product of unit vectors. Clamp FP drift.
        sim = M @ M.T  # (n, n)
        # Take the strict upper triangle (i < j).
        iu, ju = _np.triu_indices(n, k=1)
        for i, j in zip(iu.tolist(), ju.tolist()):
            out[(ids[i], ids[j])] = float(max(-1.0, min(1.0, float(sim[i, j]))))
        return out
    # Pure-Python fallback (zero-dep).
    for i in range(n):
        for j in range(i + 1, n):
            out[(ids[i], ids[j])] = _cosine_similarity(embs[i], embs[j])
    return out
...
def bulk_link_cards(
    ...
    """Bulk-seed the graph for a batch of cards in ONE vectorized pass.

    Replaces the naive per-``add()`` O(n²) pattern — where the store loaded
    ALL embeddings and called ``auto_link_cards`` once per insert (n loads × n
    cosines = O(n²) loads + O(n²) cosines). This computes the full pairwise
    cosine matrix with a single ``M @ M.T`` matmul and writes every
    above-threshold edge in one transaction, so the bulk-seed is O(n²) cosines
    but **O(1) embedding loads** — the load was the dominant cost at scale
    (the audit's RSS probe measured ~44s + 236MB for the naive path at 10k).

    Args:
        conn: the store's connection.
        cards: ``[(card_id, tags, embedding_or_None)]``. Cards with ``embedding
            is None`` are skipped for the cosine pass but still participate in
            the shared-tag pass.
        cosine_threshold: min cosine to create a ``"semantic"`` edge.

    Returns:
        Number of edges written (cosine + shared-tag).
    """
    if len(cards) < 2:
        return 0
    now = time.time()
    ...
    cur = conn.cursor()
    try:
        # ---- cosine pass: one vectorized pairwise matmul ----------------- #
        ids = [c[0] for c in cards if c[2] is not None]
        embs = [c[2] for c in cards if c[2] is not None]
        pairs = _pairwise_cosine(ids, embs)
        ...
        # ---- shared-tag pass: batched Jaccard over non-empty tag sets ------ #
        tagged = [(c[0], set(c[1])) for c in cards if c[1]]
        for i in range(len(tagged)):
            id_i, tags_i = tagged[i]
            for j in range(i + 1, len(tagged)):
                id_j, tags_j = tagged[j]
                union = len(tags_i | tags_j)
                if union and (len(tags_i & tags_j) / union) > 0.0:
                    jac = len(tags_i & tags_j) / union
                    cur.execute(
                        "INSERT INTO card_edges (source_id, target_id, relation_type, weight, created_at) "
                        "VALUES (?, ?, 'shared_tag', ?, ?) "
                        "ON CONFLICT(source_id, target_id, relation_type) DO UPDATE SET weight=excluded.weight",
...
def auto_link_cards(
    ...
    if embedding is not None:
        # Vectorized fast path: one matmul of the (n, dim) matrix against the
        # query vector beats the per-pair pure-Python loop when numpy is up.
        other_ids = [oid for oid, _ in all_embeddings if oid != card_id]
        other_embs = [oe for oid, oe in all_embeddings if oid != card_id]
        if _HAS_NUMPY and other_embs:
            q = _np.asarray(embedding, dtype=_np.float32)
            M = _np.asarray(other_embs, dtype=_np.float32)
            if M.ndim == 2 and M.shape[0]:
                scores = M @ q  # (n,)
                for k, oid in enumerate(other_ids):
                    s = float(max(-1.0, min(1.0, float(scores[k]))))
                    if s > cosine_threshold:
                        insert_edge(conn, card_id, oid, "semantic", s)
                        linked.add(oid)
        else:
            for other_id, other_emb in zip(other_ids, other_embs):
