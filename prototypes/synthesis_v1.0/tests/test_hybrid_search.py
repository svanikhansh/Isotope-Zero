...
class TestFTS5Sync:
    ...
    def test_invalidate_vector_cache_after_direct_write(self, store):
        # External/direct-connection writers bypass add()/update(), so the
        # Python-side vector + scope caches go stale. invalidate_vector_cache
        # (the vector-side analogue of rebuild_fts) must make a card written
        # via the raw connection visible to the next hybrid_search.
        store.add(_card("c1", "the user likes sqlite", embedding=_unit_vec(0)))
        # Direct-connection insert (the eval-harness bulk-seeder path): no
        # _mark_vec_dirty fires, so the matrix + scope masks stay stale.
        ...
        store._conn.execute(
            ...
            ("c2", "the user likes sqlite too", "e", 2.0, 5, "[]", array("f", v).tobytes()),
        )
        store.rebuild_fts()  # FTS stays correct via triggers + rebuild
        # Without invalidation, c2 is invisible to the vector branch:
        store.invalidate_vector_cache()
        hits = store.hybrid_search("sqlite user", _unit_vec(1), k=5)
        ids = [c.id for c, _ in hits]
        assert "c2" in ids, (
            "direct-connection insert must be visible to hybrid_search after "
            "invalidate_vector_cache (the vector cache rebuild picks it up)"
        )
