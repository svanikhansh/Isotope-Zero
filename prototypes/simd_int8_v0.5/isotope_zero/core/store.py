...
from isotope_zero.core.quantization import (
    dequantize_vector,
    int8_dot_product,
    quantize_vector,
)
...
def _resolve_native_simd():
    """Lazily import the native NEON/AVX int8 SIMD kernel.

    Returns a callable bound method of the native module, or ``None`` if the
    extension is unavailable / the function is missing. Cached at module
    scope so we never re-import per query. The store reads the result once
    into ``self._native_simd`` at first use.

    We keep this at module scope (not on the store instance) so the import
    side-effect + attribute lookup happens exactly once across all store
    instances.
    """
    fn = getattr(_resolve_native_simd, "_fn", None)
    if fn is not None:
        return fn
    try:
        import isotope_zero._native as _native

        fn = getattr(_native, "simd_int8_batch_dot", None)
    except Exception:
        fn = None
    _resolve_native_simd._fn = fn  # type: ignore[attr-defined]
    return fn
...
class MemoryStore:
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def __init__(self, db_path: str = ":memory:", embedder: Any = None) -> None:
        ...
        # SQ8 quantized-cache: a parallel (n, dim) int8 matrix + per-row float
        # scales, mirroring the float32 cache. Used by ``vector_search_int8``.
        # Same dirty/hetero pattern. ``_qvec_norms`` caches the dequantized
        # L2 norms per row so the cosine rescale is O(n) not O(n*d) per query.
        # ``_qvec_dirty`` is wired to ``_mark_vec_dirty`` (both caches share the
        # same dirty flag): any write that invalidates the float32 cache also
        # invalidates the int8 cache, since they cover the same row set.
        self._qvec_matrix = None
        self._qvec_scales = None
        self._qvec_ids = None
        self._qvec_ts = None
        self._qvec_norms = None
        self._qvec_dirty = True
        self._qvec_hetero = False
        # Native NEON/AVX int8 SIMD kernel (``isotope_zero._native``).
        # Resolved lazily ONCE on first ``vector_search_int8`` use and cached
        # for the store's lifetime so we never re-import per query. ``None``
        # when the extension is absent/unbuilt -> the pure-Python
        # ``int8_dot_product`` path is used instead (correctness preserved).
        self._native_simd: object | None = False
    ...
    def _init_schema(self) -> None:
        ...
        with self._lock:
            ...
            # SQ8 migration: add the quantized-vector columns. ``q_embedding``
            # holds the packed int8 bytes (``np.int8(...).tobytes()``) and
            # ``q_scale`` the per-vector float scale. Idempotent via the same
            # PRAGMA table_info guard. Pre-existing rows have NULL here and are
            # backfilled lazily from ``embedding`` on the next qvec cache build
            # (see ``_ensure_qvec_cache``), so no DB rebuild is required and
            # the suite stays green on existing DBs.
            if "q_embedding" not in cols:
                cur.execute("ALTER TABLE memories ADD COLUMN q_embedding BLOB")
            if "q_scale" not in cols:
    ...
    @staticmethod
    def _encode_qembedding(
        vec: list[float] | None,
    ) -> tuple[bytes | None, float | None]:
        """Quantize an embedding to int8 bytes + a per-vector scale.

        Returns ``(None, None)`` when ``vec`` is None (so NULL embeddings get
        SQL NULL in both ``q_embedding`` and ``q_scale``, mirroring the
        float32 column). Otherwise returns ``(q_vec.tobytes(), scale)`` where
        ``q_vec`` is a ``np.int8`` array and ``scale`` a Python float.

        numpy is the only SQ8 dependency; if numpy is importable enough for
        ``vector_search_int8`` it is importable here. We import locally so a
        numpy-absent environment still constructs the store and runs the
        float32/SQL paths unchanged.
        """
        if vec is None:
            return None, None
        import numpy as _np

        q_vec, scale = quantize_vector(_np.asarray(vec, dtype=_np.float32))
        return q_vec.tobytes(), scale
    ...
    def add(self, card: MemoryCard) -> None:
        ...
        Also quantizes the embedding to int8 and stores ``q_embedding`` (packed
        int8 bytes) + ``q_scale`` (per-vector float) for the
        ``vector_search_int8`` path. ``card.embedding`` itself stays a float32
        list on the card and in the ``embedding`` column, so ``get``/``all``/
        ``batch_get`` still return float32 cards (no existing call site breaks).
        """
        ...
        q_blob, q_scale = self._encode_qembedding(card.embedding)
        ...
        with self._lock:
            ...
            cur.execute(
                """
                INSERT INTO memories(id, fact, evidence, timestamp, tags, source_tokens, embedding, access_count, last_access, superseded_by, q_embedding, q_scale)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    card.id,
    ...
    def update(self, card: MemoryCard) -> None:
        ...
        Like ``add``, also writes the int8 quantized bytes + scale so the
        quantized cache stays consistent with the float32 embedding.
        """
        tags_json = json.dumps(list(card.tags)) if card.tags else None
        blob = self._encode_embedding(card.embedding)
        q_blob, q_scale = self._encode_qembedding(card.embedding)
    ...
    def _mark_vec_dirty(self) -> None:
        ...
        Also invalidates the parallel SQ8 quantized cache (``_qvec_*``), since
        the two caches cover the same row set: any write that forces a
        float32 rebuild forces an int8 rebuild too. Touch does not touch the
        qvec cache either, for the same reason.
        """
        self._vec_dirty = True
        self._vec_hetero = False
        self._qvec_dirty = True
    ...
    def _ensure_qvec_cache(self, np) -> Any:
        """Build/return the cached (n, dim) int8 quantized matrix + scales.

        Mirrors ``_ensure_vec_cache``: SELECTs (id, timestamp, q_embedding,
        q_scale, embedding) for rows with non-NULL q_embedding AND
        superseded_by IS NULL. Decodes each int8 BLOB via
        ``np.frombuffer(blob, dtype=np.int8)``, stacks into
        ``self._qvec_matrix`` (n, d) int8, and builds
        ``self._qvec_scales`` (n,) float32. Also precomputes
        ``self._qvec_norms`` -- the L2 norms of the DEQUANTIZED vectors
        (``np.linalg.norm(scale[:,None] * q_matrix.astype(float32),
        axis=1)``) -- so the per-query cosine rescale is O(n) not O(n*d).

        BACKFILL (documented): rows that have a non-NULL float32
        ``embedding`` but a NULL ``q_embedding`` (e.g. pre-existing rows from
        before the SQ8 migration, OR rows inserted by the eval harness's
        ``_seed_bulk`` which writes directly to the connection listing only
        the original 9 columns) are quantized on the fly from ``embedding``
        so the quantized cache is complete and matches the float32 cache's
        row set. The backfill is in-memory only (the DB columns stay NULL);
        the next ``add``/``update`` for such a row persists the quantized
        bytes. This keeps ``vector_search_int8`` usable on bulk-seeded
        benchmark stores without a DB rebuild.

        Returns None when there are no quantizable rows or when rows have
        heterogeneous int8 lengths (not matrix-able -> caller falls back to
        the float32 ``vector_search``).
        """
        if self._qvec_hetero:
            return None
        if (not self._qvec_dirty) and self._qvec_matrix is not None:
            return self._qvec_matrix

        with self._lock:
            cur = self._conn.cursor()
            # Select q_embedding + q_scale for rows that already have them,
            # PLUS the float32 embedding for rows that don't (backfill path).
            # Both filtered to superseded_by IS NULL so the row set matches
            # _ensure_vec_cache exactly (a fair 4x footprint comparison).
            cur.execute(
                "SELECT id, timestamp, q_embedding, q_scale, embedding "
            ...
            rows = cur.fetchall()
            cur.close()
            # Clear the dirty flag while still holding the lock (same rationale
            # as _ensure_vec_cache: a write landing after this point re-dirties
            # the flag, forcing a rebuild on the next call).
            self._qvec_dirty = False
        ...
        scales: list[float] = []
        dim: int | None = None
        for rid, rts, q_blob, q_scale, f32_blob in rows:
            if q_blob is not None:
                # Persisted quantized bytes: use them directly.
                q = np.frombuffer(q_blob, dtype=np.int8)
                scale = float(q_scale) if q_scale is not None else 1.0
            else:
                # Backfill: quantize the float32 embedding on the fly. f32_blob
                # is non-NULL here (WHERE clause guarantees embedding IS NOT
                # NULL).
                f32 = np.frombuffer(f32_blob, dtype=np.float32)
                q, scale = quantize_vector(f32)
            if dim is None:
                dim = q.shape[0]
            elif q.shape[0] != dim:
                # Heterogeneous int8 lengths are not matrix-able; latch and
                # let the search fall back to the float32 path.
                self._qvec_hetero = True
                self._qvec_matrix = None
                return None
            ids.append(rid)
        ...
        self._qvec_ts = ts
        if q_arrs:
            self._qvec_matrix = np.stack(q_arrs)  # (n, d) int8
            self._qvec_scales = np.asarray(scales, dtype=np.float32)
            # Precompute dequantized L2 norms for the cosine rescale.
            # dequant_i = scale_i * q_i (float32); norm = ||dequant_i||.
            dequant = self._qvec_matrix.astype(np.float32) * self._qvec_scales[:, None]
            self._qvec_norms = np.linalg.norm(dequant, axis=1).astype(np.float32)
        else:
            self._qvec_matrix = None
            self._qvec_scales = None
            self._qvec_norms = None
    ...
    def vector_search_int8(
        self, query_vec: list[float], k: int = 5
    ) -> list[tuple[MemoryCard, float]]:
        """Quantized top-k cosine search: int8 dot-product over the cached
        quantized matrix.

        Quantizes the query (``query_q, query_scale = quantize_vector(query)``)
        and runs ``int8_dot_product(self._qvec_matrix, query_q,
        self._qvec_scales, query_scale)`` to get the approximate RAW dot
        product of the dequantized vectors per row. The raw dot is NOT a
        cosine: the int8 reconstruction is not re-L2-normalized, so we
        rescale to a comparable [0,1] cosine score via::

            score_i = raw_dot_i / (||dequant_vec_i|| * ||dequant_query||)

        where ``||dequant_vec_i||`` is precomputed per cache build
        (``self._qvec_norms``) and ``||dequant_query||`` is computed once
        per query (``np.linalg.norm(query_scale * query_q.astype(float32))``).
        Because the original float32 inputs were L2-normalized, this
        approximate cosine is directly comparable to ``vector_search``'s
        [0,1] scores -- the benchmark's rank-correlation test is meaningful.

        Ordering and return type are IDENTICAL to ``vector_search``:
        (score desc, timestamp asc, id asc), ``list[(MemoryCard, float)]``.
        Falls back to ``vector_search`` (full float32 BLAS path) when the
        quantized cache is unavailable -- numpy missing, heterogeneous dims,
        empty cache. Empty/all-zero ``query_vec`` -> ``[]`` (mirrors
        ``vector_search``). Superseded cards are excluded (the cache query
        filters ``superseded_by IS NULL``).

        Honest latency note: the dot step runs through the native NEON int8
        SIMD kernel (``isotope_zero._native.simd_int8_batch_dot``) when the
        extension is built -- a zero-copy numpy read + GIL-released Rust loop.
        If the native extension is absent/unbuilt the call falls back to the
        pure-Python numpy ``int8_dot_product`` (a non-BLAS integer matmul),
        which IS slower than ``vector_search``. The fallback exists for
        correctness, not to hide slowness.
        """
        ...
        qmatrix = self._ensure_qvec_cache(np)
        if qmatrix is None or qmatrix.shape[0] == 0:
            # No searchable rows OR heterogeneous dims. Fall back to the
            # float32 BLAS path (correct for both: empty -> [], hetero ->
            # per-card min-dim dot).
            return self.vector_search(query_vec, k)
        ...
        com = qmatrix.shape[1]
        # Length-mismatch path mirrors vector_search: compare over the first
        # min(qdim, emb_dim) components. (Rare for the 384-dim embedder, but
        ...
        # Quantize the query and compute its dequantized norm (per-query).
        query_q, query_scale = quantize_vector(q)
        query_dequant = query_q.astype(np.float32) * np.float32(query_scale)
        query_norm = float(np.linalg.norm(query_dequant))
        if query_norm == 0.0:
            return []

        # Raw approximate dot products (scale_i * query_scale * int_dot).
        # Prefer the native NEON/AVX int8 SIMD kernel (zero-copy numpy reads +
        # GIL-released Rust loop); fall back to the pure-Python numpy
        # ``int8_dot_product`` if the extension is absent/unbuilt or if the
        # native call raises. Both compute the same math
        # (scales[i]*query_scale*sum_j(q_matrix[i,j]*q_query[j])) and return a
        # float32 (n,) array of RAW dots -- the downstream cosine-rescale /
        # clip / sort logic is IDENTICAL for both paths.
        native_fn = self._native_simd
        if native_fn is False:
            # One-time lazy resolution: cache the bound callable (or None).
            native_fn = _resolve_native_simd()
            self._native_simd = native_fn
        raw = None
        if native_fn is not None:
            try:
                raw = native_fn(qmatrix, query_q, self._qvec_scales, query_scale)
            except Exception:
                # Any native failure (dtype/shape mismatch, etc.) MUST fall
                # back to the correctness-guaranteed pure-Python path rather
                # than propagate -- search is never broken by the native path.
                raw = None
        if raw is None:
            raw = int8_dot_product(qmatrix, query_q, self._qvec_scales, query_scale)
        raw = np.asarray(raw, dtype=np.float32)
        # Cosine rescale: raw / (row_norm * query_norm). row_norms are
        # precomputed in _ensure_qvec_cache. Guard divide-by-zero (a row whose
        # dequant norm is 0 is an all-zero vector -- skip via the clip below).
        row_norms = self._qvec_norms[: qmatrix.shape[0]]
        denom = row_norms * np.float32(query_norm)
        # Avoid division by zero: where denom==0, set score to 0.
        safe = denom > 0.0
        scores = np.zeros(raw.shape, dtype=np.float32)
        np.divide(raw, denom, out=scores, where=safe)
        np.clip(scores, 0.0, 1.0, out=scores)

        total = qmatrix.shape[0]
        kk = min(k, total) if k > 0 else 0
        if kk <= 0:
            return []

        # Same top-k + tie-expand + (score desc, timestamp asc, id asc) sort as
        # vector_search, so ranking is directly comparable.
        cand = np.argpartition(scores, -kk)[-kk:]
        thr = float(scores[cand].min())
        cand = np.flatnonzero(scores >= thr)
        entries = [
            (float(scores[i]), self._qvec_ts[i], self._qvec_ids[i])
    ...
    # ------------------------------------------------------------------ #
    # SQ8 footprint reporting
    # ------------------------------------------------------------------ #
    def quantized_matrix_nbytes(self) -> int:
        """Resident size in bytes of the int8 quantized cache (the honest SQ8
        footprint). Builds the cache first (via ``_ensure_qvec_cache``) so the
        number is real, not stale. Returns 0 when numpy is missing or the
        cache is empty/unbuildable (hetero dims). For a 10k-card x 384-dim
        store this is ~3.84 MB vs ~15.36 MB for the float32 cache (4x smaller).
        """
        try:
            import numpy as np
        except ImportError:
            return 0
        m = self._ensure_qvec_cache(np)
        if m is None:
            return 0
        return int(m.nbytes)

    def float32_matrix_nbytes(self) -> int:
        """Resident size in bytes of the float32 BLAS cache (the comparison
        baseline). Builds the cache first (via ``_ensure_vec_cache``) so the
        number is real. Returns 0 when numpy is missing or the cache is
        empty/unbuildable (hetero dims).
        """
        try:
            import numpy as np
        except ImportError:
            return 0
        m = self._ensure_vec_cache(np)
        if m is None:
            return 0
        return int(m.nbytes)
