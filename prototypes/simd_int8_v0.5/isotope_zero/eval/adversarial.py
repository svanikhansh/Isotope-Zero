...
class ScaleResult:
    ...
    # -- Method 2: Int8 SQ8 quantized vector search vs float32 baseline --
    # int8_search latency percentiles (ms). The brief makes NO specific latency
    # claim for int8 vs float32 (int8 matmul is not BLAS-accelerated, so it is
    # EXPECTED to be slower); we report both honestly and surface the
    # int8_slower_than_float32 convenience for the comparison table.
    int8_p50_ms: float = 0.0
    int8_p95_ms: float = 0.0
    int8_p99_ms: float = 0.0
    # Matrix footprints (MB). These build the respective caches.
    float32_matrix_mb: float = 0.0
    int8_matrix_mb: float = 0.0
    # Brief Method 2 target: int8 cache < 4.0 MB at 10k cards (~75% reduction).
    claim_int8_matrix_mb: float = 4.0
    # Brief Method 2 target: rank correlation > 98% with raw float32 cosine.
    claim_rank_corr: float = 0.98
    # Measured Spearman rank correlation of int8 vs float32 cosine scores over
    # the FULL ranked list (all cards, not just top-k) for the benchmark query.
    rank_corr: float = 0.0
    # True when the store agent's int8 methods were absent at run time, so the
    # int8/footprint/correlation numbers are all 0.0 and the int8 claims are
    # treated as NOT measured (rendered N/A, never silently PASS).
    int8_unavailable: bool = False
    # -- Method 4: Native NEON/AVX-VNNI int8 SIMD kernel (the point of this
    # prototype). The store's `vector_search_int8` routes through the native
    # `isotope_zero._native.simd_int8_batch_dot` when the kernel is built, with
    # fallback to the pure-Python `int8_dot_product`. We measure TWO numbers:
    # the end-to-end `simd_p99_ms` (vector_search_int8 with native routing,
    # including GIL/cache-lookup/sort/batch_get store overhead) and the raw
    # `simd_kernel_p99_ms` (direct `_native.simd_int8_batch_dot` call on the
    # cached `_qvec_matrix`/`_qvec_scales`, isolating the NEON matmul from store
    # overhead). The raw-kernel number is the truest measure of whether NEON
    # beats BLAS; the end-to-end number is what a user of the store sees.
    simd_kernel: str = ""  # e.g. "neon-sdot" / "avx2-vnni"; "" when unavailable
    simd_unavailable: bool = False
    simd_p50_ms: float = 0.0
    simd_p95_ms: float = 0.0
    simd_p99_ms: float = 0.0  # end-to-end vector_search_int8 via native routing
    simd_kernel_p99_ms: float = 0.0  # raw _native.simd_int8_batch_dot only
    # Brief Method 4 target: native SIMD kernel p99 < 0.10 ms. This claim is on
    # the RAW KERNEL (`simd_kernel_p99_ms`), not the end-to-end store path
    # (`simd_p99_ms`) -- the store overhead (GIL, cache lookup, sort, batch_get)
    # is orthogonal to whether NEON beats BLAS. Both are reported honestly.
    claim_simd_p99_ms: float = 0.10
    ...
    @property
    def int8_matrix_claim_holds(self) -> bool:
        # A claim cannot "hold" if the method that produces the measurement was
        # never available: report FAIL rather than let 0.0 < 4.0 pass silently.
        if self.int8_unavailable:
            return False
        return self.int8_matrix_mb < self.claim_int8_matrix_mb

    @property
    def rank_corr_claim_holds(self) -> bool:
        if self.int8_unavailable:
            return False
        return self.rank_corr >= self.claim_rank_corr

    @property
    def int8_slower_than_float32(self) -> bool:
        """Convenience for the comparison table: is int8 p99 SLOWER than the
        float32 BLAS path? (Expected True for the pure-Python Method 2 path;
        Expected False for the native Method 4 path.)"""
        if self.int8_unavailable:
            return False
        return self.int8_p99_ms > self.vector_p99_ms

    @property
    def simd_p99_claim_holds(self) -> bool:
        """Method 4 native-kernel target: raw SIMD p99 < claim_simd_p99_ms.

        Measured against `simd_kernel_p99_ms` (the raw `_native` call), NOT the
        end-to-end `simd_p99_ms` (which carries store overhead orthogonal to
        whether NEON beats BLAS). N/A (FAIL) when the native kernel is not
        built/ready OR when no measurement was taken (kernel p99 is exactly
        0.0, which only happens when the kernel was never timed) -- we never
        fabricate a passing threshold."""
        if self.simd_unavailable:
            return False
        if self.simd_kernel_p99_ms <= 0.0:
            return False
        return self.simd_kernel_p99_ms < self.claim_simd_p99_ms
...
def _spearman_rank_corr(a: list[float], b: list[float]) -> float:
    """Spearman rank correlation of two equal-length score arrays.

    Uses ``scipy.stats.spearmanr`` when scipy is importable; otherwise falls
    back to a pure-numpy rank-Pearson (rank each array via argsort, then
    Pearson correlation of the ranks — mathematically equivalent to Spearman).
    The fallback is documented because this venv has no scipy. Returns 0.0
    on degenerate input (length mismatch, constant array -> zero variance).
    """
    n = min(len(a), len(b))
    if n < 2:
        return 0.0
    ...
    try:
        ...
        pass
    # Pure-numpy fallback: rank each array (average ranks for ties via
    # argsort + searchsorted), then Pearson on the ranks.
    import numpy as np
    ...
    def _ranks(x: list[float]) -> "np.ndarray":
        ...
        while i < n:
            j = i
            # gather all values tied with arr[order[i]]
            while j + 1 < n and arr[order[j + 1]] == arr[order[i]]:
                j += 1
            avg_rank = (i + j) / 2.0 + 1.0  # 1-based average rank over [i, j]
...
def run_scale(n: int, embedder: EmbeddingEngine, reps: int = 30) -> ScaleResult:
    ...
    # -- Method 2: Int8 SQ8 quantized vector search vs float32 baseline --
    # Same store, same cards, same idx, same q_vec as the float32 timing above,
    # so the comparison is apples-to-apples. Every int8 number below is N/A
    # (and the int8 claims FAIL) when the store agent's int8 methods are not
    # yet present: we never fabricate a measurement.
    int8_samples: list[float] = []
    int8_p = [0.0, 0.0, 0.0]
    f32_matrix_mb = 0.0
    int8_matrix_mb = 0.0
    rank_corr = 0.0
    int8_unavailable = not (
        hasattr(store, "vector_search_int8")
        and hasattr(store, "quantized_matrix_nbytes")
        and hasattr(store, "float32_matrix_nbytes")
    )
    if not int8_unavailable:
        # Warm the int8 path (builds its cache) before timing.
        store.vector_search_int8(q_vec, k=5)
        for _ in range(reps):
            t0 = time.perf_counter(); store.vector_search_int8(q_vec, k=5)
            int8_samples.append(time.perf_counter() - t0)
        int8_p = _percentiles(int8_samples, (50, 95, 99))
        # Footprints: these build the respective caches; call once AFTER the
        # warm-up above so both caches are populated (nbytes is the size of the
        # populated cache, not a pre-build estimate).
        f32_matrix_mb = store.float32_matrix_nbytes() / 1024.0 / 1024.0
        int8_matrix_mb = store.quantized_matrix_nbytes() / 1024.0 / 1024.0
        # Rank correlation of int8 vs float32 cosine over the FULL ranked list
        # (all n cards, not just top-k) so the correlation is meaningful over
        # the whole ranking. Both methods return list[(card, score)] sorted by
        # score desc; build id->score maps, align by id, then Spearman.
        full = min(n, store.count())
        f32_hits = store.vector_search(q_vec, k=full)
        i8_hits = store.vector_search_int8(q_vec, k=full)
        f32_by = {c.id: sc for c, sc in f32_hits}
        i8_by = {c.id: sc for c, sc in i8_hits}
        shared = [cid for cid in f32_by if cid in i8_by]
        if len(shared) >= 2:
            f32_scores = [f32_by[cid] for cid in shared]
            i8_scores = [i8_by[cid] for cid in shared]
            rank_corr = _spearman_rank_corr(f32_scores, i8_scores)

    # -- Method 4: Native NEON/AVX-VNNI int8 SIMD kernel (the point of this
    # prototype). Detect whether the native module is importable AND the kernel
    # function actually executes (a stub that raises NotImplementedError counts
    # as NOT available -- we never fabricate a passing threshold). When live,
    # time the end-to-end `vector_search_int8` (which the store routes through
    # the native kernel with fallback) AND a raw-kernel micro-bench that calls
    # `_native.simd_int8_batch_dot` directly on the cached quantized matrix to
    # isolate the NEON matmul from store overhead (GIL, cache lookup, sort,
    # batch_get). Both numbers are reported; the raw-kernel p99 is what the
    # <0.10 ms claim is measured against.
    simd_kernel = ""
    simd_unavailable = False
    simd_samples: list[float] = []
    simd_kernel_samples: list[float] = []
    simd_p = [0.0, 0.0, 0.0]
    simd_kernel_p99 = 0.0
    try:
        import isotope_zero._native as _native  # type: ignore
        has_fn = hasattr(_native, "simd_int8_batch_dot")
        has_name = hasattr(_native, "simd_kernel_name")
        # Probe the kernel with a tiny array to distinguish a real implementation
        # from a stub that exists but raises NotImplementedError. The probe mirrors
        # the store's call shape: (q_matrix (n,d) int8, q_query (d,) int8,
        # scales (n,) f32, query_scale f32) -> (n,) f32 raw dots.
        if has_fn:
            import numpy as _np_probe
            _pm = _np_probe.zeros((2, 8), dtype=_np_probe.int8)
            _pq = _np_probe.zeros(8, dtype=_np_probe.int8)
            _ps = _np_probe.ones(2, dtype=_np_probe.float32)
            _probe = _native.simd_int8_batch_dot(_pm, _pq, _ps, 1.0)
            # A live kernel returns a real array; a stub raises before here.
            if not hasattr(_probe, "shape"):
                raise RuntimeError("native simd_int8_batch_dot returned non-array")
        else:
            raise RuntimeError("native simd_int8_batch_dot not exported")
        if has_name:
            simd_kernel = str(_native.simd_kernel_name())
        else:
            # Kernel is live but the name accessor hasn't landed yet: report a
            # neutral label so the SIMD row still renders with real numbers.
            simd_kernel = "native (unnamed)"
    except Exception:
        # ImportError (module not built), NotImplementedError (stub), or any
        # probe failure -> the native SIMD path is not live this run. The SIMD
        # row renders N/A and the <0.10 ms claim FAILS (never silently passes).
        simd_unavailable = True

    if not simd_unavailable:
        # End-to-end: time vector_search_int8 (which the store routes through
        # the native kernel with fallback). This is the SAME call the Method 2
        # int8 timing above made, but now the store's int8 path is native. We
        # re-warm (the store may have switched its routing) and re-time for the
        # SAME reps so the comparison is apples-to-apples with the float32 path.
        store.vector_search_int8(q_vec, k=5)  # warm
        for _ in range(reps):
            t0 = time.perf_counter(); store.vector_search_int8(q_vec, k=5)
            simd_samples.append(time.perf_counter() - t0)
        simd_p = _percentiles(simd_samples, (50, 95, 99))

        # Raw-kernel micro-bench: call _native.simd_int8_batch_dot DIRECTLY on
        # the cached _qvec_matrix / _qvec_scales (the same arrays the store
        # would pass). This isolates the NEON matmul from store overhead
        # (cache lookup, quantize query, rescale, sort, batch_get). The query
        # is pre-quantized once outside the timing loop so we measure ONLY the
        # kernel, not the per-call quantize. The cached matrix is guaranteed
        # populated by the warm-up above.
        import numpy as _np_k
        from isotope_zero.core.quantization import quantize_vector as _qvec
        _kmatrix = store._qvec_matrix
        _kscales = store._qvec_scales
        _kquery_q, _kquery_scale = _qvec(_np_k.asarray(q_vec, dtype=_np_k.float32))
        if _kmatrix is not None and _kmatrix.shape[0] > 0:
            # Warm the raw kernel path (first call may pay one-time dispatch cost).
            _native.simd_int8_batch_dot(_kmatrix, _kquery_q, _kscales, _kquery_scale)
            for _ in range(reps):
                t0 = time.perf_counter()
                _native.simd_int8_batch_dot(_kmatrix, _kquery_q, _kscales, _kquery_scale)
                simd_kernel_samples.append(time.perf_counter() - t0)
            simd_kernel_p99 = _percentiles(simd_kernel_samples, (99,))[0]
        # If the cached matrix is None (hetero/empty), the raw-kernel number
        # stays 0.0 and the claim FAILS via the property's divide-by-zero guard
        # is not the issue -- simd_unavailable stays False but kernel_p99 is 0.0,
        # which is < 0.10 and would PASS falsely. Guard explicitly:
        if _kmatrix is None or _kmatrix.shape[0] == 0:
            simd_unavailable = True  # cannot measure the raw kernel honestly
    ...
    return ScaleResult(
        ...
        rss_mb=rss,
        int8_p50_ms=int8_p[0], int8_p95_ms=int8_p[1], int8_p99_ms=int8_p[2],
        float32_matrix_mb=f32_matrix_mb, int8_matrix_mb=int8_matrix_mb,
        rank_corr=rank_corr, int8_unavailable=int8_unavailable,
        simd_kernel=simd_kernel, simd_unavailable=simd_unavailable,
        simd_p50_ms=simd_p[0], simd_p95_ms=simd_p[1], simd_p99_ms=simd_p[2],
        simd_kernel_p99_ms=simd_kernel_p99,
    )
...
def render_adversarial_markdown(res: AdversarialResult) -> str:
    ...
    lines.append(f"| int8_vector_search p99 (pure-Python M2) | {s.int8_p99_ms:.2f} ms | (no claim — vs float32 {s.vector_p99_ms:.2f} ms, {'int8 slower' if s.int8_slower_than_float32 else 'int8 faster/not slower'}) | "
                 f"{'N/A' if s.int8_unavailable else 'INFO'} |")
    ...
    lines.append(f"| float32 matrix footprint | {s.float32_matrix_mb:.2f} MB | (no claim — baseline) | {'N/A' if s.int8_unavailable else 'INFO'} |")
    lines.append(f"| int8 matrix footprint | {s.int8_matrix_mb:.2f} MB | <{s.claim_int8_matrix_mb} MB | {'N/A' if s.int8_unavailable else ('PASS' if s.int8_matrix_claim_holds else 'FAIL')} |")
    lines.append(f"| rank correlation int8 vs f32 | {s.rank_corr:.4f} | >{s.claim_rank_corr} | {'N/A' if s.int8_unavailable else ('PASS' if s.rank_corr_claim_holds else 'FAIL')} |")
    lines.append(f"| **native SIMD kernel p99 (raw)** | **{s.simd_kernel_p99_ms:.3f} ms** | <{s.claim_simd_p99_ms} ms | {'N/A' if s.simd_unavailable else ('PASS' if s.simd_p99_claim_holds else 'FAIL')} |")
    lines.append(f"| native SIMD end-to-end p99 (via store) | {s.simd_p99_ms:.3f} ms | (no claim — raw kernel {s.simd_kernel_p99_ms:.3f} ms + store overhead) | {'N/A' if s.simd_unavailable else 'INFO'} |")
    lines.append(f"| native SIMD kernel | {s.simd_kernel or '(none)'} | (probe) | {'N/A' if s.simd_unavailable else 'INFO'} |")
    lines.append("")
    if s.int8_unavailable:
        lines.append("_int8 SQ8 methods (vector_search_int8 / quantized_matrix_nbytes / "
                     "float32_matrix_nbytes) not present on MemoryStore — int8/footprint/correlation "
                     "rows are N/A. The Method 2 deliverables will populate once the store agent lands them._")
        lines.append("")
    if s.simd_unavailable:
        lines.append("_native SIMD kernel (isotope_zero._native.simd_int8_batch_dot) not built/ready "
                     "— the kernel probe raised (stub NotImplementedError or import failure). "
                     "SIMD rows are N/A and the <0.10 ms claim FAILS. The native SIMD row will populate "
                     "once the kernel agent implements the NEON/AVX kernel, adds simd_kernel_name, and "
                     "wires the store to call it with fallback._")
        lines.append("")
    lines.append(f"vector p50/p95/p99 = {s.vector_p50_ms:.2f}/{s.vector_p95_ms:.2f}/{s.vector_p99_ms:.2f} ms; "
                 f"int8 p50/p95/p99 = {s.int8_p50_ms:.2f}/{s.int8_p95_ms:.2f}/{s.int8_p99_ms:.2f} ms; "
                 f"simd p50/p95/p99 = {s.simd_p50_ms:.3f}/{s.simd_p95_ms:.3f}/{s.simd_p99_ms:.3f} ms (raw kernel p99 = {s.simd_kernel_p99_ms:.3f} ms); "
    ...
    passed = sum([s.vector_claim_holds, s.sql_claim_holds, s.rss_claim_holds,
                  s.int8_matrix_claim_holds, s.rank_corr_claim_holds,
                  s.simd_p99_claim_holds,
                  n.recall_claim_holds, g.negation_claim_holds, c.concurrency_claim_holds])
    lines.append(f"**Total: {passed}/9 claims hold.** (harness wall-clock {res.latency_ms/1000.0:.1f}s)")
    lines.append("")
    lines.append("## Method 4 — Float32 BLAS vs Pure-Python int8 (M2) vs Native SIMD int8 (M4)")
    lines.append("")
    lines.append("Three-way honest comparison. The pure-Python int8 column is the Method 2")
    lines.append("baseline (non-BLAS integer matmul, expected slower than float32). The native")
    lines.append("SIMD column is the Method 4 deliverable: NEON `vdotq_s32` on this M4, measured")
    lines.append("both end-to-end (through the store) and raw (direct `_native` call).")
    lines.append("")
    if s.int8_unavailable:
        lines.append("Store int8 methods unavailable — comparison table cannot be measured yet.")
        lines.append("")
        return "\n".join(lines)
    lines.append("| dimension | float32 BLAS (M1) | pure-Python int8 (M2) | native SIMD int8 (M4) | verdict |")
    lines.append("|---|---|---|---|---|")
    # Footprint row: int8 cache is identical for M2 and M4 (the kernel changes compute, not storage).
    lines.append(f"| matrix footprint @ {s.n_cards} cards | {s.float32_matrix_mb:.2f} MB | {s.int8_matrix_mb:.2f} MB | {s.int8_matrix_mb:.2f} MB | "
                 f"{'PASS (<4.0 MB)' if s.int8_matrix_claim_holds else 'FAIL'} "
                 f"(~{s.float32_matrix_mb / s.int8_matrix_mb:.1f}× smaller than f32) |")
    # Latency row: float32 p99 vs pure-Python int8 p99 vs native SIMD raw-kernel p99.
    if s.simd_unavailable:
        lines.append(f"| p99 latency (raw kernel) | {s.vector_p99_ms:.2f} ms | {s.int8_p99_ms:.2f} ms | N/A (kernel not ready) | "
                     f"float32 {'PASS' if s.vector_claim_holds else 'FAIL'} <{s.claim_vector_ms}; "
                     f"SIMD {'FAIL (unavailable)' if not s.simd_p99_claim_holds else 'PASS'} <{s.claim_simd_p99_ms} |")
        lines.append(f"| p99 latency (end-to-end via store) | {s.vector_p99_ms:.2f} ms | {s.int8_p99_ms:.2f} ms | N/A | (no claim — report only) |")
    else:
        lines.append(f"| p99 latency (raw kernel) | {s.vector_p99_ms:.2f} ms (BLAS matmul) | {s.int8_p99_ms:.2f} ms (numpy int loop) | {s.simd_kernel_p99_ms:.3f} ms ({s.simd_kernel}) | "
                     f"SIMD {'PASS' if s.simd_p99_claim_holds else 'FAIL'} <{s.claim_simd_p99_ms} ms |")
        lines.append(f"| p99 latency (end-to-end via store) | {s.vector_p99_ms:.2f} ms | {s.int8_p99_ms:.2f} ms | {s.simd_p99_ms:.3f} ms | "
                     f"{'SIMD faster than f32' if s.simd_p99_ms < s.vector_p99_ms else 'SIMD slower than f32'} by {abs(s.simd_p99_ms - s.vector_p99_ms):.3f} ms |")
    # Rank correlation row: the math is identical between M2 and M4 (only the matmul backend changes),
    # so the same rank_corr applies to both. f32 is self (1.0).
    lines.append(f"| rank corr vs f32 cosine | 1.0000 (self) | {s.rank_corr:.4f} | {s.rank_corr:.4f} | "
                 f"{'PASS (>0.98)' if s.rank_corr_claim_holds else 'FAIL'} (claim >{s.claim_rank_corr}) |")
    lines.append("")
    return "\n".join(lines)
    ...
    if args.json:
        ...
        print(json.dumps({
            "scale": {**res.scale.__dict__,
                      ...
                      "int8_matrix_claim_holds": res.scale.int8_matrix_claim_holds,
                      "rank_corr_claim_holds": res.scale.rank_corr_claim_holds,
                      "int8_slower_than_float32": res.scale.int8_slower_than_float32,
                      "simd_p99_claim_holds": res.scale.simd_p99_claim_holds},
