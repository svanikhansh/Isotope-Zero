...
class MemoryStore:
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def __init__(self, db_path: str = ":memory:", embedder: Any = None) -> None:
        ...
        # Binary (1-bit) quantized vector cache for 2-stage POPCNT search.
        # Lazily built from the float32 matrix on first use; invalidated
        # alongside the float32 cache by _mark_vec_dirty().
        self._vec_binary_matrix = None  # packed uint8 (n, 48) lazy cache
        self._vec_binary_dirty = True
    ...
    def _mark_vec_dirty(self) -> None:
        ...
        The binary (1-bit) cache is invalidated alongside the float32
        cache because it derives from the same underlying embedding rows.
        """
        self._vec_dirty = True
        self._vec_hetero = False
        self._vec_binary_matrix = None
        self._vec_binary_dirty = True
    ...
    def _ensure_binary_cache(self, np) -> Any:
        """Return the cached (n, 48) uint8 packed binary matrix, building on demand.

        Derived from the float32 matrix via 1-bit sign-quantization (packbits).
        Returns None when ``_ensure_vec_cache`` returns None (no embeddable rows
        or heterogeneous embedding lengths).  The binary cache shares the same
        dirtiness lifecycle as the float32 cache: ``_mark_vec_dirty`` clears both.
        """
        if (not self._vec_binary_dirty) and self._vec_binary_matrix is not None:
            return self._vec_binary_matrix
        ...
        self._vec_binary_matrix = quantize_1bit(matrix)
        self._vec_binary_dirty = False
        return self._vec_binary_matrix

    def vector_search_binary_rerank(
        self,
        query_vec: list[float],
        k: int = 5,
        oversample_factor: int = 10,
    ) -> list[tuple[MemoryCard, float]]:
        """2-stage binary -> float32 vector search.

        **Stage 1 (POPCNT)**: 1-bit sign-quantized vectors (384-dim packed as
        48-byte uint8) are compared via XOR + Hamming distance. This selects the
        top ``k * oversample_factor`` candidates from the full index using a
        fused CPU intrinsic kernel when available, falling back to numpy.

        **Stage 2 (re-rank)**: Exact float32 cosine similarity (BLAS matmul) on
        ONLY the surviving candidates, clamped to [0, 1], with the same
        tie-breaking rule as ``vector_search``: (score desc, timestamp asc,
        id asc).

        If the binary cache cannot be built (no embeddable rows, heterogeneous
        dims, or missing ``binary_quant`` module) the call transparently falls
        back to ``self.vector_search(query_vec, k)``.

        Args:
            query_vec: L2-normalized embedding vector (list[float]).
            k: Number of top results to return.
            oversample_factor: Stage-1 candidate pool multiplier.
        """
        ...
        try:
            from isotope_zero.core import binary_quant  # noqa: F811
        except ImportError:
            return self.vector_search(query_vec, k)

        float32_matrix = self._ensure_vec_cache(np)
        binary_matrix = self._ensure_binary_cache(np)
        if float32_matrix is None or binary_matrix is None or binary_matrix.shape[0] == 0:
            return self.vector_search(query_vec, k)
        ...
        # --- Stage 1: quantize query + POPCNT candidate selection ---
        q_packed = binary_quant.quantize_1bit(q.reshape(1, -1))[0]  # uint8 (48,)
        n_rows = binary_matrix.shape[0]
        top_n = min(k * oversample_factor, n_rows)

        # Try the native SIMD POPCNT kernel first; fall back to numpy.
        try:
            from isotope_zero._native import (  # type: ignore[import-not-found,import-untyped]
                popcnt_hamming_search as _native_popcnt,
            )

            native_idx, _native_dists = _native_popcnt(
                binary_matrix, q_packed, k, oversample_factor,
            )
            candidate_indices = list(native_idx)
        except (ImportError, AttributeError, TypeError, Exception):
            idx_list, _dists_list = binary_quant.binary_search(
                float32_matrix, q, k, oversample_factor,
            )
        ...
        # --- Stage 2: float32 cosine re-rank ---
        candidate_matrix = float32_matrix[candidate_indices]  # (n_cands, dim)
        scores = candidate_matrix @ q  # BLAS dot product
        np.clip(scores, 0.0, 1.0, out=scores)

        total_cands = scores.shape[0]
        kk = min(k, total_cands) if k > 0 else 0
        if kk <= 0:
            return []

        # Top-k by score; expand to every row tied at the k-th boundary so
        # timestamp tie-breaking matches `vector_search` exactly.
        ...
        entries = [
            (
                float(scores[i]),
                self._vec_ts[candidate_indices[i]],
                self._vec_ids[candidate_indices[i]],
            )
