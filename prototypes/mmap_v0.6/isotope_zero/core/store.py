...
class MemoryStore:
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def __init__(
        self, db_path: str = ":memory:", embedder: Any = None,
        use_mmap: bool = True,
    ) -> None:
        ...
        Args:
            ...
            use_mmap: When True (default), `_ensure_vec_cache` builds an
                `MmapVectorStore` (file-backed `np.memmap` in
                `embeddings.bin`) instead of `np.stack(arrs)`, and
                `vector_search` runs `mmap_matrix @ q` (the memmap supports
                `@`; numpy reads pages on demand from the backing file). The
                heap path is the fallback when `use_mmap=False` OR the dims
                are heterogeneous OR the mmap build fails for any reason.
                See `isotope_zero.core.mmap_store` for the honest memory
                model (mmap moves ~matrix-bytes out of the heap but a full
                scan pages them back in; the ONNX runtime dominates total
                RSS regardless).
        """
        ...
        # Method 3: file-backed mmap backend. Built lazily by
        # `_ensure_vec_cache` when `use_mmap` is True; `None` means the heap
        # path is in use (disabled, hetero, or build-failed). Owned by this
        # store and invalidated alongside `_vec_dirty`.
    ...
    def _mark_vec_dirty(self) -> None:
        ...
        Method 3: also invalidates the `MmapVectorStore` (file-backed
        memmap) so its stale file + hot LRU are never read after a write.
        The memmap is rebuilt on the next `_ensure_vec_cache`.
        """
        self._vec_dirty = True
        self._vec_hetero = False
    ...
    def _ensure_vec_cache(self, np) -> Any:
        ...
        Method 3 (mmap path): when `use_mmap=True`, the matrix is a
        file-backed `np.memmap` built by `MmapVectorStore.build` instead of
        `np.stack(arrs)`. The memmap is a drop-in: it supports `@` and
        `[:, :n]` slicing identically to a heap ndarray (numpy reads pages
        on demand from the backing file). The parallel `self._vec_ids` /
        `self._vec_ts` are populated from the mmap store's index so the
        post-score top-k selection is identical to the heap path. On ANY
        failure (hetero dim, build error, or `use_mmap=False`), the heap
        `np.stack` path is the fallback — the caller sees the same matrix
        contract either way.
        """
        if self._vec_hetero:
            return None
        ...
        # Method 3: try the file-backed mmap backend first when enabled.
        # The mmap store does its own SELECT + write + memmap-open under
        # our lock (we pass the held connection). On hetero-dim or build
        # failure it raises _HeteroDim / a generic Exception; we fall back
        # to the heap path so search still works (and latches hetero).
        if self.use_mmap:
            try:
                from isotope_zero.core.mmap_store import MmapVectorStore, _HeteroDim
                if self._mmap_store is None:
                    self._mmap_store = MmapVectorStore(
                        self.db_path, self._conn,
                    )
                with self._lock:
                ...
                if mm is not None:
                    # Populate the parallel id/ts lists from the mmap store
                    # so vector_search's top-k selection is identical to the
                    # heap path (same row order, same timestamps, same ids).
                    self._vec_ids = list(self._mmap_store.ids)
                    self._vec_ts = list(self._mmap_store.ts)
                    self._vec_matrix = mm
                    return mm
                # mm is None -> no embeddable rows. Fall through to the heap
                # path, which also returns None for an empty result, keeping
                # _vec_matrix consistent (None signals "no searchable rows").
            except _HeteroDim:
                # Heterogeneous dims: latch and let vector_search fall back
                # to the pure-Python loop (which handles per-card min-dim).
                ...
                # Any mmap build failure (disk full, permission, mmap
                # unsupported on the platform, ...) -> fall back to the
                # heap path. Do NOT latch hetero (the data may be uniform;
                # the mmap path just couldn't open the file).
                self._mmap_store = None
                # _vec_dirty still True; the heap path clears it below.
    ...
    def vector_search(
        ...
        # Method 3: warm the Hot LRU with the top-k ids so the NEXT query
        # for the same/nearby vectors hits the resident cache instead of
        # re-faulting those memmap rows. The full `matrix @ q` already ran
        # (pages faulted in on demand); this only populates the cache for
        # future queries. No-op when the heap path is in use (no mmap
        # store). The memmap's pages are NOT resident in the heap — the hot
        # cache stores COPIES of the rows so they survive page eviction.
        if self._mmap_store is not None:
            try:
                self._mmap_store.hot_warm(want_ids, np)
            except Exception:
                pass  # cache warming must never break a search
    ...
    def close(self) -> None:
        ...
        with self._lock:
            # Method 3: close the mmap store (flush + drop the memmap) before
            # closing the SQLite connection the mmap store borrows.
            if self._mmap_store is not None:
                try:
