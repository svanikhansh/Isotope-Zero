...
from isotope_zero.core.decay import calculate_retention
...
class Consolidator:
    ...
    def _are_duplicates(self, a: MemoryCard, b: MemoryCard) -> bool:
        ...
        if _are_negations_guard(a.fact, b.fact):
            return False
    ...
    def _merge_graph_cluster(self, cluster_ids: list[str], compound_weights: dict[str, float] | None = None) -> tuple[MemoryCard, list[str]]:
        """Merge a graph cluster into a single survivor (centrality-based selection).

        Survivor selection: the card with the highest ``compound_weight``
        (graph centrality hub).  Ties are broken by ``access_count``, then by
        most-recent ``timestamp``, then by id for determinism.

        Merge rules:
          - fact: concatenated with ``"; "``, truncated to 200 chars.
          - evidence: first 2 distinct fragments via ``_merge_evidence(cap=2)``.
          - embedding: re-embedded from the merged fact (if embedder available).
          - stability: max of cluster members (defaults to 1.0 if not set).
          - importance: max of cluster members (defaults to 0.0 if not set).
          - tags: union, preserving order, deduped.
          - access_count: sum across cluster.
          - last_access: max across cluster.
        """
        cards = self.store.batch_get(cluster_ids)
        # Filter out any cards that were deleted between the cluster detection
        # and the batch fetch (should not happen under the store lock, but
        # defensive).
        cards = [c for c in cards if c is not None]
        if not cards:
            raise ValueError("_merge_graph_cluster: no cards found for cluster")
        if len(cards) < 2:
            return cards[0], []

        # Survivor: highest compound_weight, then access_count, then timestamp.
        def _survivor_key(c: MemoryCard) -> tuple[float, int, float, str]:
            if compound_weights is not None:
                cw = compound_weights.get(c.id, 0.0)
            else:
                cw = float(graph.compound_weight(self.store._conn, c.id))
            return (cw, c.access_count, c.timestamp, c.id)

        survivor_card = max(cards, key=_survivor_key)
        folded = [c for c in cards if c.id != survivor_card.id]

        # Merge facts; concatenate distinct facts with "; " separator.
        seen_facts: set[str] = {survivor_card.fact.strip()}
        ordered: list[str] = [survivor_card.fact]
        for c in folded:
            sf = c.fact.strip()
            if sf not in seen_facts:
                seen_facts.add(sf)
                ordered.append(sf)
        merged_fact = "; ".join(ordered)
        if len(merged_fact) > 200:
            merged_fact = merged_fact[:197] + "..."

        # Evidence: first 2 distinct fragments.
        evidence_frags: list[str] = [survivor_card.evidence]
        evidence_frags.extend(c.evidence for c in folded)
        merged_evidence = _merge_evidence(evidence_frags, cap=2)

        # Stability: max of cluster members.
        max_stability = max(
            (getattr(c, "stability", 1.0) for c in cards), default=1.0
        )

        # Importance: max of cluster members.
        max_importance = max(
            (getattr(c, "importance", 0.0) for c in cards), default=0.0
        )
        ...
        # Sum access pressure; take most-recent last_access across cluster.
        sum_access = sum(c.access_count for c in cards)
        max_last_access = max((c.last_access for c in cards), default=0.0)

        # Re-embed the merged fact if we have a working embedder.
        emb = survivor_card.embedding
        if self.embedder is not None:
            try:
                re = self.embedder.embed_text(merged_fact)
                if re:
                    emb = re
            except Exception as exc:  # defensive: never let re-embed kill a sweep
                log.warning("re-embed failed for graph survivor %s: %s", survivor_card.id, exc)
    ...
    def run(self) -> ConsolidationReport:
        ...
        # 3. Plan decay (Ebbinghaus). Score every card that is NOT already
        # folded by dedup. For a cluster, the survivor (canonical id) is
        # what we score, using its merged access_count/last_access so a
        # cluster that was recalled is never pruned.
        # Ensure graph table exists + batch-load compound weights under the
        # store lock so we never race with another thread touching the connection.
        with self.store._lock:
            graph.init_graph(self.store._conn)
            cur = self.store._conn.execute(
                "SELECT source_id, SUM(weight) FROM card_edges GROUP BY source_id"
            )
            compound_weights: dict[str, float] = dict(cur.fetchall())
        pruned_ids: list[str] = []
        ...
        for card in all_cards:
            ...
            if scoring_card.access_count > 0:
                # Recalled cards are never pruned, regardless of retention.
                continue
            last_ts = scoring_card.last_access if scoring_card.last_access else scoring_card.timestamp
            retention = calculate_retention(last_ts)
            cw = float(compound_weights.get(cid, 0.0))
            if retention < 0.15 and cw == 0.0:
                pruned_ids.append(cid)
                pruned_vitalities.append(retention)
                pruned_set.add(cid)

        # 3.5. Graph summarization — merge tight graph clusters into summary
        # cards. Runs after dedup + decay so already-removed cards are skipped.
        with self.store._lock:
            clusters = graph.detect_clusters(self.store._conn, min_cluster_size=3, min_edge_weight=0.80)
        if clusters:
            for cluster_ids in clusters:
                active = [cid for cid in cluster_ids
                           if cid not in dedup_deleted_set and cid not in pruned_set]
                if len(active) < 3:
                    continue
                graph_survivor, folded_ids = self._merge_graph_cluster(active, compound_weights)
                survivors.append(graph_survivor)
                dedup_deleted.extend(folded_ids)
                for fid in folded_ids:
                    superseded_map[fid] = graph_survivor.id
...
# ------------------------------------------------------------------ #
# High-level entry point for Phase 7C graph+decay consolidation
# ------------------------------------------------------------------ #


def consolidate_memory_space(
    db_path: str,
    ...
    *,
    ...
    Opens the DB, runs one full Ebbinghaus+graph consolidation sweep,
    and returns a summary dict.

    Args:
        db_path: Path to the SQLite database file (or ``":memory:"``).
        retention_threshold: Ebbinghaus retention below which a card is a
            pruning candidate (default 0.15).
        embedder: Optional embedding engine for re-embedding merged survivors.

    Returns:
        dict with keys: ``pruned_count``, ``clusters_merged``,
        ``survivors_created``, ``edges_before``, ``edges_after``,
        ``active_cards_before``, ``active_cards_after``,
        ``compression_ratio``.
    """
    from isotope_zero.core import graph as graph_module

    store = MemoryStore(db_path, embedder=embedder)
    graph_module.init_graph(store._conn)

    stats_before = graph_module.get_graph_stats(store._conn)
    active_before = store.count()

    consolidator = Consolidator(store, embedder)
    report = consolidator.run()

    stats_after = graph_module.get_graph_stats(store._conn)
    active_after = store.count()

    compression_ratio = 1.0 - (active_after / max(1, active_before)) if active_before > 0 else 0.0

    return {
        "pruned_count": report.decayed_cards,
        "clusters_merged": report.merged_cards,
        "survivors_created": report.merged_cards,  # each cluster fold counts as survivor creation
        "edges_before": stats_before["edge_count"],
        "edges_after": stats_after["edge_count"],
        "active_cards_before": active_before,
        "active_cards_after": active_after,
