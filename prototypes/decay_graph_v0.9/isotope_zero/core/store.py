...
_SQL_LOOKUP_SELECT = (
    ...
    "embedding, access_count, last_access, superseded_by, stability, importance, archived"
)
...
_SQL_EXACT = (
    ...
    "WHERE superseded_by IS NULL AND archived = 0 AND {col} = ? COLLATE NOCASE "
...
_SQL_SUBSTR_IDS = "SELECT id FROM memories INDEXED BY idx_fact WHERE {col} LIKE ? COLLATE NOCASE AND superseded_by IS NULL AND archived = 0"
...
_SQL_TAGS_SCAN = (
    "SELECT {select} FROM memories NOT INDEXED "
    "WHERE superseded_by IS NULL AND archived = 0 ORDER BY timestamp ASC, id ASC"
)
...
class MemoryStore:
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def _row_to_card(row: tuple[Any, ...]) -> MemoryCard:
        ...
        last_access, superseded_by, stability, importance, archived.
    ...
    def add(self, card: MemoryCard) -> None:
        ...
        with self._lock:
            ...
            cur.execute(
                """
                INSERT INTO memories(id, fact, evidence, timestamp, tags, source_tokens, embedding, access_count, last_access, superseded_by, stability, importance, archived)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    card.id,
    ...
    def batch_get(self, ids: list[str]) -> list[MemoryCard]:
        ...
        with self._lock:
            ...
            cur.execute(
                f"""
                SELECT id, fact, evidence, timestamp, tags, source_tokens, embedding, access_count, last_access, superseded_by, stability, importance, archived
    ...
    def all(self) -> list[MemoryCard]:
        """Return every live (non-archived, non-superseded) card, ordered by timestamp ascending."""
        with self._lock:
            cur = self._conn.cursor()
            cur.execute(
                """
                SELECT id, fact, evidence, timestamp, tags, source_tokens, embedding, access_count, last_access, superseded_by, stability, importance, archived
                FROM memories NOT INDEXED WHERE superseded_by IS NULL AND archived = 0
                ORDER BY timestamp ASC, id ASC
                """
            )
            rows = cur.fetchall()
    ...
    def _ensure_vec_cache(self, np) -> Any:
        ...
        with self._lock:
            ...
            cur.execute(
                ...
                # lookups would only add random I/O).
    ...
    def vector_search(
        self, query_vec: list[float], k: int = 5, alpha: float = 0.70
        ...
        if np is None:
            return self._vector_search_fallback(query_vec, k, alpha)
        ...
        if alpha < 1.0 and entries:
            ...
            return [(by_card[e[3]], e[0]) for e in top if e[3] in by_card]
        else:
            ...
            by_card = {c.id: c for c in self.batch_get(want_ids)}
            return [(by_card[e[2]], e[0]) for e in top if e[2] in by_card]

    def _vector_search_fallback(
        self, query_vec: list[float], k: int, alpha: float = 0.70
    ) -> list[tuple[MemoryCard, float]]:
    ...
    def count(self) -> int:
        ...
        with self._lock:
            cur = self._conn.cursor()
            cur.execute("SELECT COUNT(*) FROM memories WHERE superseded_by IS NULL AND archived = 0")
            (n,) = cur.fetchone()
            cur.close()
        return int(n)
