...
class MemoryStore:
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def vector_search_int8(
        ...
        Honest latency caveat: ``int8_dot_product`` uses a non-BLAS integer
        matmul, so this method may be SLOWER than ``vector_search``. That is
        a real measurement; the fallback exists for correctness, not to hide
        slowness.
        """
