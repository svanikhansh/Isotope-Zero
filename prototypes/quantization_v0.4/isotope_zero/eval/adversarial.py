...
class ScaleResult:
    ...
    def int8_slower_than_float32(self) -> bool:
        ...
        float32 BLAS path? (Expected True — int8 matmul is not BLAS.)"""
        if self.int8_unavailable:
            return False
...
def render_adversarial_markdown(res: AdversarialResult) -> str:
    ...
    lines.append(f"| int8_vector_search p99 | {s.int8_p99_ms:.2f} ms | (no claim — vs float32 {s.vector_p99_ms:.2f} ms, {'int8 slower' if s.int8_slower_than_float32 else 'int8 faster/not slower'}) | "
    ...
    lines.append(f"**Total: {passed}/8 claims hold.** (harness wall-clock {res.latency_ms/1000.0:.1f}s)")
    lines.append("")
    lines.append("## Method 2 — Float32 vs Int8 SQ8 (honest comparison)")
    lines.append("")
    if s.int8_unavailable:
        lines.append("Store int8 methods unavailable — comparison table cannot be measured yet.")
    else:
        lines.append("| dimension | float32 (BLAS) | int8 SQ8 | verdict |")
        lines.append("|---|---|---|---|")
        lines.append(f"| matrix footprint @ {s.n_cards} cards | {s.float32_matrix_mb:.2f} MB | {s.int8_matrix_mb:.2f} MB | "
                     f"{'PASS (<4.0 MB)' if s.int8_matrix_claim_holds else 'FAIL'} "
                     f"(~{s.float32_matrix_mb / s.int8_matrix_mb:.1f}× smaller if int8>0) |")
        lines.append(f"| vector p99 latency | {s.vector_p99_ms:.2f} ms | {s.int8_p99_ms:.2f} ms | "
                     f"{'int8 slower' if s.int8_slower_than_float32 else 'int8 faster'}"
                     f" by {abs(s.int8_p99_ms - s.vector_p99_ms):.2f} ms (no claim — report only) |")
        lines.append(f"| rank corr vs f32 cosine | 1.0000 (self) | {s.rank_corr:.4f} | "
