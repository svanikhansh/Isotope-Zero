...
from . import native
# The Phase 7C graph engine lives in ``graph.relation_graph`` as a module of
# plain functions (init_graph / auto_link_cards / compound_weight / ...); bind
# it as ``graph`` so every ``graph.<fn>`` call site below resolves.
from ..graph import relation_graph as graph
from .decay import calculate_retention, update_stability, hybrid_score
    ...
    **REQUIRED for multi-process / multi-connection deployments:** this store
    ...
    def batch_get(self, ids: list[str]) -> list[MemoryCard]:
        ...
        with self._lock:
            ...
            cur.execute(
                f"""
                SELECT id, fact, evidence, timestamp, tags, source_tokens, embedding, access_count, last_access, superseded_by, stability, importance, archived, scope
    ...
    def _ensure_vec_cache(self, np) -> Any:
        ...
        if self.use_mmap:
            try:
                from .mmap_store import MmapVectorStore, _HeteroDim
