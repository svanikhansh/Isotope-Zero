"""isotope_zero CLI — clean, human-readable output (the "plain" render path).

Every ``_cmd_*`` in ``debug.py`` has two output paths: a machine contract (``--json``,
untouched here and the source of truth for scripts/SDK/MCP) and a human-readable path.
This module owns the **human-readable** path and gives it a single, Mem0-style voice:

    the fact text is the hero. ids, scores, vitality floats, token counts and
    ``=== isotope_zero <verb> ===`` banners are dropped from the default view and
    surfaced only behind ``--verbose`` for power users.

Every function here is **pure** — it takes data and returns a ``str``; it does no I/O
of its own. Callers (``debug.py``) ``print()`` the result. That makes the output
contract trivially testable, mirroring ``menu._banner`` / ``menu._numbered_menu`` and
``dashboard._render_frame`` (pure formatters, tested without a terminal).

Encoding: the default glyphs are a small set of Unicode marks (``✓`` ok, ``·`` bullet,
``→`` arrow). On a non-UTF-8 stdout (Windows cp1252 in a non-unicode code page, or a
piped ``LC_ALL=C`` locale) those would raise ``UnicodeEncodeError``. ``_glyph()``
falls back to ASCII equivalents (``ok``, ``-``, ``->``) when ``sys.stdout`` can't
encode the Unicode form, so the clean output works in every terminal — the same
graceful-degradation philosophy the menu/dashboard use for ``rich``.
"""
