...
if _NATIVE_SO_PATH is None:
    _here = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
    _candidate_sib = _os.path.join(_here, "_native_int8", "isotope_zero", "_native.abi3.so")
    if _os.path.exists(_candidate_sib):
        _NATIVE_SO_PATH = _candidate_sib
    else:
        # Development/repo install: walk up to the repo root and locate the
        # research prototype. Only resolved if the file actually exists.
        _root = _here
        for _ in range(6):
            _repo = _os.path.dirname(_root)
            if _os.path.basename(_root) == "src":
                _proto = _os.path.join(_os.path.dirname(_root), "prototypes",
                                      "simd_int8_v0.5", "isotope_zero",
                                      "_native.abi3.so")
                if _os.path.exists(_proto):
                    _NATIVE_SO_PATH = _proto
                break
            _root = _repo
...
def _try_import_native_kernel() -> "Callable[..., Any] | None":
    ...
    """
    if not _NATIVE_SO_PATH or not _os.path.exists(_NATIVE_SO_PATH):
        log.debug("native int8 .so not found (%s); using numpy fallback",
                  _NATIVE_SO_PATH or "no candidate path")
        return None
    try:
